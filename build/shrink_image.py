...
def do_resize(args):
    ...
    with LoopDev(args, args.image) as disk_dev:
        ...
        start_sector = get_partition_info(args, disk_dev)[
            args.root_partition_number].start
        ...
        written_end = get_partition_info(args, disk_dev)[
            args.root_partition_number].end
        if written_end != new_end_sector:
            print('End sector not written correctly:', file=sys.stderr)
            print('  expected %d, read %d' %
                  (new_end_sector, written_end), file=sys.stderr)
            sys.exit(1)
...
def do_compress(args):
    ...
    subprocess.check_call(['xz', '-%d' %
                           args.compression_level, '--force', args.image])
