...
void TagConfig_read(TagConfig *cfg, FX_FILE *cfg_file){
    ...
    Option(ConfigToken) maybe_tok =__configTokenIter_next(&tok_iter);

    while(maybe_tok.some){
        ConfigToken tok = OPTION_UNWRAP(maybe_tok, return);
        switch(tok.key){
            case CFG_TOK_KEY_AUDIO_CH_0:
        ...
        maybe_tok =__configTokenIter_next(&tok_iter);
    }
}
