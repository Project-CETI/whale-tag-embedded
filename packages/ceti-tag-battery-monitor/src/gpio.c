...
int gpio_set_direction(const char * gpio_direction, const char * in_or_out) {
    ...
    if (fd == -1) {
        fprintf(stderr, "Error writing to %s", gpio_direction);
        return -1;
    }
...
int gpio_set_value(const char * gpio_value, const char * gpio_1_or_0) {
    ...
    if (fd == -1) {
        fprintf(stderr, "Unable to open %s", gpio_value);
        return -1;
    }

    if (write(fd, gpio_1_or_0, 1) != 1) {
        fprintf(stderr, "Unable to write to %s", gpio_value);
        close(fd);
        return -1;
    }
    close(fd);
...
int gpio_get_value(const char * gpio_value) {
    ...
    if (read(fd, value_str, 3) == 0) {
        fprintf(stderr, "Unable to read from %s", gpio_value);
        close(fd);
        return -1;
    }
    close(fd);
