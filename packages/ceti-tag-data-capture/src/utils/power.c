...
void activity_led_disable(void){
    ...
    if(led_trigger_file < 0){
        CETI_LOG("Could not open activity led trigger file: " ACT_LED_TRIGGER_SYSFS_PATH);
    } else {
    ...
    if(led_brightness_file < 0){
        CETI_LOG("Could not open activity led brightness file: " ACT_LED_BRIGHTNESS_SYSFS_PATH);
    } else {
