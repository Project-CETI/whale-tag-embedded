...
static const char *goPros_data_file_headers[] = {"GoPro Index", "Command",
                                                 "Message Ack Time [us]"};
...
int init_goPros() {
  ...
  // Initialize the Python communication (paths are relative to the bin
  // directory where this program runs).
  system(
      "python3 ../src/sensors/goPros_python_interface/goPros_interface.py &");
...
void *goPros_thread(void *paramPtr) {
  ...
  while (client_accepted == 0 && !g_stopAcquisition) {
    goPros_python_connected_socket =
        accept(goPros_python_socket, (struct sockaddr *)&goPros_python_client,
               &goPros_python_client_length);
    client_accepted = (goPros_python_connected_socket >= 0);
    if (client_accepted == 0)
      usleep(200000);
  }
  CETI_LOG("Accepted client connection from Python program (%s)",
           inet_ntoa(goPros_python_client.sin_addr));
  ...
  while (!g_stopAcquisition) {
    ...
    // active.
    if (get_global_time_us() - last_python_heartbeat_us >=
        GOPRO_PYTHON_HEARTBEAT_PERIOD_US) {
      ...
      send_goPros_message(&goPro_python_message,
                          sizeof(goPro_python_message_struct));
      last_python_heartbeat_us = get_global_time_us();
    }
    // Periodically flash a heartbeat on the LEDs.
    if (get_global_time_us() - last_led_heartbeat_us >=
        GOPRO_LED_HEARTBEAT_PERIOD_US) {
      if (led_heartbeat_on_us == -1) {
        gpioWrite(GOPRO_LED_GPIO_GREEN, PI_ON);
        led_heartbeat_on_us = get_global_time_us();
      }
      if (get_global_time_us() - led_heartbeat_on_us >=
          (goPros_are_recording ? led_heartbeat_recording_duration_us
                                : led_heartbeat_stopped_duration_us)) {
        gpioWrite(GOPRO_LED_GPIO_GREEN, PI_OFF);
        last_led_heartbeat_us =
            get_global_time_us() - (goPros_are_recording
                                        ? led_heartbeat_recording_duration_us
                                        : led_heartbeat_stopped_duration_us);
        led_heartbeat_on_us = -1;
      }
    }

    // Determine whether the magnet is present, debouncing the result to make a
    // stable verdict. The magnet pulls the GPIO low.
    ...
    magnet_detected_debounced =
        magnet_detection_count >= magnet_detection_count_threshold;
    ...
    if (!magnet_detected_debounced && prev_magnet_detected_debounced) {
      ...
      for (int i = 0; i < 3 && !goPro_command_success && !g_stopAcquisition;
           i++) {
        if (!goPros_are_recording) {
          CETI_LOG("Starting %d GoPros", NUM_GOPROS);
          goPro_command_success =
...
int create_goPros_socket() {
  ...
  if (bind(goPros_python_socket, (struct sockaddr *)&goPros_python_server,
           sizeof(goPros_python_server)) < 0)
    return -1;
...
int send_command_to_all_goPros(int goPro_command) {
  ...
  for (int goPro_index = 0; goPro_index < NUM_GOPROS; goPro_index++) {
    ...
    while (received_client_ack == 0 && get_global_time_us() - timeout_start_us <
                                           GOPRO_PYTHON_ACK_TIMEOUT_US) {
      num_read_from_client =
          read(goPros_python_connected_socket, goPros_python_buffer,
               GOPRO_PYTHON_BUFFERSIZE);
      if (num_read_from_client > 0) {
        goPro_python_message =
            *((goPro_python_message_struct *)goPros_python_buffer);
