...
int init_goPros()
{
  ...
  system("python /home/bckim//GoPro/whale/whale-tag-embedded/packages/ceti-tag-data-capture/src/goPros_interface.py &"); // should be changed after deployment
  usleep(5000000);                                                                                                       // used to wait until python starts
...
void send_command_to_all_goPros(int goPro_command)
{
  ...
  for (int goPro_index = 0; goPro_index < NUM_GOPROS; goPro_index++)
  {
    ...
    while (received_client_ack == 0 && get_global_time_us() - timeout_start_us < 1000000) // it means it waits 10sec to get the ack signal -> It should be increased sometime
    {
