...
int ecg_gpio_expander_setup(int i2c_bus) {
  ...
  ecg_gpio_expander_i2c_device =
      i2cOpen(i2c_bus, ECG_GPIO_EXPANDER_I2C_ADDRESS, 0);
  if (ecg_gpio_expander_i2c_device < 0) {
    CETI_LOG("XXX Failed to connect to the GPIO expander: returned %d.",
             ecg_gpio_expander_i2c_device);
  ...
  // but will also cause strong pullups to be briefly connected (during HIGH
  // time of the acknowledgment pulse).
  // To avoid this for now, the default state is leveraged and no commands are
  // sent.

  // Set all to inputs (1 = weakly driven high)
...
// Will first read all inputs of the GPIO expander, then extract the desired
// bit.
...
// Given a byte of all GPIO expander inputs, extract the ECG leads-off
// detection bit (positive electrode).
int ecg_gpio_expander_parse_leadsOff_p(uint8_t data) {
  return (data >> ECG_GPIO_EXPANDER_CHANNEL_LEADSOFF_P) & 0b00000001;
}

// Given a byte of all GPIO expander inputs, extract the ECG leads-off
// detection bit (negative electrode).
...
void ecg_gpio_expander_set_leds_green() {
  ...
  i2cWriteByte(ecg_gpio_expander_i2c_device,
               (uint8_t) ~(1 << (ECG_GPIO_EXPANDER_CHANNEL_LEDGREEN)));
#endif
}
...
void ecg_gpio_expander_set_leds_yellow() {
  ...
  // Setting all other bits to 1 will keep all other channels as inputs.
  i2cWriteByte(ecg_gpio_expander_i2c_device,
               (uint8_t) ~(1 << (ECG_GPIO_EXPANDER_CHANNEL_LEDYELLOW)));
#endif
}
...
void ecg_gpio_expander_set_leds_red() {
  ...
  // Setting all other bits to 1 will keep all other channels as inputs.
  i2cWriteByte(ecg_gpio_expander_i2c_device,
               (uint8_t) ~(1 << (ECG_GPIO_EXPANDER_CHANNEL_LEDRED)));
#endif
}
