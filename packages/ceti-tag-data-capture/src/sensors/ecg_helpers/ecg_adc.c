...
void ecg_adc_cleanup() {
  ...
  // Commenting the below since the launcher will call gpioTerminate() as part of the tag-wide cleanup.
...
void ecg_adc_set_data_rate(int rate) {
  ...
  switch (rate) {
    case (20): ecg_adc_config |= ECG_ADC_DATA_RATE_20; break;
    case (90): ecg_adc_config |= ECG_ADC_DATA_RATE_90; break;
    case (330): ecg_adc_config |= ECG_ADC_DATA_RATE_330; break;
    case (1000): ecg_adc_config |= ECG_ADC_DATA_RATE_1000; break;
    default: break;
  }
  ecg_adc_config_apply();
}
...
#if ECG_ADC_DATA_READY_USE_INTERRUPT
  // Cancel the callback.
  int res = gpioSetISRFunc(ECG_ADC_DATA_READY_PIN, FALLING_EDGE, ECG_ADC_DATA_READY_INTERRUPT_TIMEOUT_MS, NULL);
  switch (res) {
    case PI_BAD_GPIO: CETI_LOG("XXX PI_BAD_GPIO"); break;
    case PI_BAD_EDGE: CETI_LOG("XXX PI_BAD_EDGE"); break;
    case PI_BAD_ISR_INIT: CETI_LOG("XXX PI_BAD_ISR_INIT"); break;
...
int ecg_adc_read_data(int *exit_flag, int64_t timeout_us) {
...
int32_t ecg_adc_read_singleEnded(int channel, int *exit_flag, int64_t timeout_us) {
  ...
  switch (channel) {
    case (0): ecg_adc_config |= ECG_ADC_MUX_SINGLE_0; break;
    case (1): ecg_adc_config |= ECG_ADC_MUX_SINGLE_1; break;
    case (2): ecg_adc_config |= ECG_ADC_MUX_SINGLE_2; break;
    case (3): ecg_adc_config |= ECG_ADC_MUX_SINGLE_3; break;
    default: break;
  }
