...
// Part 2 - Client commands
	...
	if ( !strncmp(g_command,"configFPGA",10) ) {
		...
		if( !loadFpgaBitstream() ) {
			...
			g_rsp = fopen(RSP,"w");
			fprintf(g_rsp,"hdlCmd(): Configuring FPGA Failed - Try Again\n"); //echo it back
		}
		fclose(g_rsp);
		return 0;
	}
	...
	if ( !strncmp(g_command,"sr_sim",6) ) {
		...
		pTemp1 = strchr(g_command,' '); //find space char - separates the params in the script
		...
		for(i=0;i<n;i++)
			cArg[i] = *(pTemp1+i); //this is the period in microseconds as a string
...
int loadFpgaBitstream(void)
{
	...
	if(pConfig == NULL) {
			fprintf(stderr,"loadFpgaBitstream(): Failed to allocate memory for the configuration file\n");
			return 1;
	}
...
//  - Host (the Pi) sends and opcode, arguments and payload (see design doc for opcode defintions)
//  - FPGA receives, executes the opcode and returns a pointer to the response string
//-----------------------------------------------------------------------------
void cam(unsigned int opcode,
