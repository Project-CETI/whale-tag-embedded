...
//         //update screen
//         int width = (tui_get_screen_width() - 13) - 1 ;
//         for(int i = 0; i < 3; i++){
//             int line = 5 + 3*i;

//             printf("\e[%d;1HCH%d: %5.3f: ", line, 1 + i, ((double)max_audio[i])/0x7FFF);
//             tui_draw_horzontal_bar(((double)max_audio[i])/0x7FFF, 1.0, 13, line, width);
...
//         int lead_state;
//         if(wt_iox_read_register(WT_IOX_REG_INPUT) != WT_OK)
//             return TEST_STATE_FAILED;
...
//         tui_draw_horzontal_bar(vis_light, 2048, 17, 5, width);
...
//         tui_draw_horzontal_bar(ir_light, 4096, 17, 8, width);
...
//         //print line at threshhold
//         tui_draw_horzontal_bar((pressure_bar > 5.0) ? 5.0 : pressure_bar, 5.0, 18, 5, width);
 ...
 */
int main(void) {
    WTResult hw_init = wt_initialize();
    if (hw_init != WT_OK) {
        CETI_ERR("%s", wt_strerror(hw_init));
        return 1;
    }
    ...
    printf("Date: %s\n", buffer); //date
    printf("\e[%d;1HPress any key to continue (esq - quit)",tui_get_screen_height());
    char input = 0;
    while((read(STDIN_FILENO, &input, 1) != 1) && (input == 0)){
        ;
    }
    ...
    for(int i = 0; i < TEST_COUNT; i++){
        ...
        printf("\e[%d;1H(enter - continue; esc - quit)\e[2;1H",tui_get_screen_height());
